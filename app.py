...
def render_overview(fdf, year_sel):
    ...
    with k[2]:
        if pd.isna(err_pct):
            ...
            sub, cls = "Close to plan", "cx-delta-pos"
        elif err_pct > 0:
            sub, cls = "Sold more than planned (under-forecast)", "cx-delta-pos"
        else:
            sub, cls = "Sold less than planned (over-forecast)", "cx-delta-neg"
...
def chart_accuracy_by_month(fdf):
    ...
    fig.update_layout(showlegend=False, title="Forecast Error by Month")
...
def chart_accuracy_by_zone(fdf):
    ...
    fig.update_layout(title="Forecast Error by Supply Chain Zone")
    return _base_layout(fig, height=380, title="")
