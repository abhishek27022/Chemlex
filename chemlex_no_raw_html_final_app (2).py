...
# -----------------------------------------------------------------------------
# KPI grid: native Streamlit cards (no raw HTML).
# This avoids the issue where HTML card strings are printed as text in deployment.
# -----------------------------------------------------------------------------

def native_kpi_card(label: str, value: str, sub: str, icon: str, accent: str):
    with st.container(border=True):
        st.markdown(f"**{icon} {label}**")
        st.markdown(f"<span style='font-size:30px;font-weight:900;color:{accent};'>{value}</span>", unsafe_allow_html=True)
        st.caption(sub)

kpi_cols = st.columns(6, gap="small")
with kpi_cols[0]:
    native_kpi_card("Active Line Items", f"{active_items:,}", f"Selected level: {selected_level}", "▦", "#2563eb")
with kpi_cols[1]:
    native_kpi_card("Total Forecast", fmt_value(forecast, target), target, "↗", "#0b75bd")
with kpi_cols[2]:
    native_kpi_card("Total Actual", fmt_value(actual, target), target, "✓", "#16a34a")
with kpi_cols[3]:
    native_kpi_card("WAPE", fmt_pct(wape), "sum(abs_error) / sum(actual)", "≈", "#f97316")
with kpi_cols[4]:
    native_kpi_card("Forecast Alignment", fmt_pct(alignment), "Actual ÷ Forecast × 100", "%", "#059669")
with kpi_cols[5]:
    native_kpi_card("Bias", fmt_pct(bias), "(Forecast - Actual) ÷ Actual", "Δ", "#dc2626")

ins1, ins2 = st.columns(2, gap="medium")
with ins1:
    st.success("**Pull / Kanban Fixed** — $22.3M restored · 9.0% WAPE · 94.0% business accuracy. This zone is now included as an official inventory CODP stream.")
with ins2:
    st.info("**Pull / FG Restored** — $474M restored · 47.6% of demand · largest CODP zone. This is now visible in the official Supply Chain Zone filter.")
