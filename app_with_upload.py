...
def read_uploaded_or_default():
    """
    Reads a user-uploaded forecast file or falls back to default/demo data.

    Upload feature is intentionally available in two places:
    1. Main dashboard area, so users can clearly see it.
    2. Sidebar, for users who prefer the compact upload control.
    """
    st.markdown("""
    <div class='filter-panel' style='margin:16px 28px 10px 28px;'>
        <div class='panel-title' style='margin-bottom:8px;'>
            <h3>Upload Forecast File</h3>
            <span class='muted'>CSV / XLSX / XLS supported</span>
        </div>
        <div class='muted'>Preferred file: <b>dashboard_forecast_accuracy_levelled.csv</b>.
        If no file is uploaded, the app will use the default file if available, otherwise demo data.</div>
    </div>
    """, unsafe_allow_html=True)

    main_uploaded = st.file_uploader(
        "Upload forecast file here",
        type=["csv", "xlsx", "xls"],
        help="Preferred file: dashboard_forecast_accuracy_levelled.csv",
        key="main_forecast_upload",
    )
    ...
    sidebar_uploaded = st.sidebar.file_uploader(
        ...
        type=["csv", "xlsx", "xls"],
        help="Preferred file: dashboard_forecast_accuracy_levelled.csv",
        key="sidebar_forecast_upload",
    )

    uploaded = main_uploaded if main_uploaded is not None else sidebar_uploaded
...
def business_accuracy(actual, forecast):
    ...
    Handles both scalar KPI values and pandas Series/arrays.
    For scalar inputs, returns a normal float/np.nan.
    For vector inputs, returns a NumPy array aligned to the input.
    """
    actual = pd.to_numeric(actual, errors="coerce")
    forecast = pd.to_numeric(forecast, errors="coerce")

    with np.errstate(divide="ignore", invalid="ignore"):
        accuracy = np.where(forecast == 0, np.nan, (actual / forecast) * 100)

    # np.where on scalar values returns a 0-dimensional ndarray.
    # Convert it to a plain scalar so downstream KPI code does not call len() on it.
    if isinstance(accuracy, np.ndarray) and accuracy.ndim == 0:
...
def safe_float_value(value):
    """
    Safely converts scalar, list, tuple, NumPy array, or pandas Series to float.
    Returns np.nan if value is empty, missing, non-finite, or invalid.
    """
    if value is None:
        return np.nan
    ...
    if isinstance(value, pd.Series):
        ...
        if value.size == 0:
            return np.nan
        # Handles both 0-D scalar arrays and normal arrays.
...
with k4:
    accuracy_class = "kpi-green" if pd.notna(overall_accuracy_value) and 90 <= overall_accuracy_value <= 110 else "kpi-orange"
    kpi_card("Accuracy %", fmt_pct(overall_accuracy_value), "Actual ÷ Forecast × 100", accuracy_class)
