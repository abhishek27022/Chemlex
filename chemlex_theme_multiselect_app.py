...
def plotly_layout(fig, height=340):
    font_color = "#e5e7eb" if DARK_MODE else "#384357"
...
filter_cols = st.columns([1, 1, 1.25, 1.45, 1.65, 1.5])
...
with filter_cols[3]:
    plants = sorted([x for x in data["plant"].dropna().unique() if x != "All"])
    selected_plants = st.multiselect("Plant", plants, default=plants, placeholder="Select one or more plants")

with filter_cols[4]:
    cats = sorted([x for x in data["product_category_1"].dropna().unique() if x != "All"])
    selected_cats = st.multiselect("Product Category", cats, default=cats, placeholder="Select one or more categories")
...
with filter_cols2[0]:
    zones = sorted([x for x in data["codp_zone"].dropna().unique() if x != "All"])
    selected_zones = st.multiselect("CODP / Supply Chain Zone", zones, default=zones, placeholder="Select one or more zones")
with filter_cols2[1]:
...
st.caption(f"Selected filters: {len(selected_zones)} CODP zone(s), {len(selected_plants)} plant(s), {len(selected_cats)} product category(s).")
