...
<style>
    ...
    [data-testid="stSidebar"] .block-container { padding:1rem 1rem 2rem 1rem !important; }
...
with st.sidebar:
    ...
    st.caption("Upload the latest forecast backtest file generated from Synthefy / Excel.")
    uploaded = st.file_uploader(
        "Upload forecast_backtest_results.csv / .xlsx",
        type=["csv", "xlsx", "xls"],
        help="Expected fields include date, codp_zone, plant, product_category_1, group_key, actual, forecast, year, month. If no file is uploaded, the app uses the CSV available in the GitHub repo.",
    )
    st.markdown("---")
    st.caption("Accuracy shown to users = Actual ÷ Forecast × 100. Example: Actual 120 and Forecast 100 means 120% accuracy.")
