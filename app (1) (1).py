...
def _add_no_data_shading(fig, df, value_col, c):
    ...
    NOTE: Newer Plotly versions call min/max on the [x0, x1] list internally,
    which raises on pandas Timestamps ('ambiguous truth value'). Convert the
    bounds to native python datetimes before passing them in.
    """
    ...
    if not len(missing):
        return
    dates = pd.to_datetime(missing["date"]).dropna()
    if dates.empty:
        return
    x0 = dates.min().to_pydatetime()
    x1 = dates.max().to_pydatetime()
    # If only one missing month, widen the band so it's actually visible
    if x0 == x1:
        x0 = x0 - pd.Timedelta(days=15)
        x1 = x1 + pd.Timedelta(days=15)
    ...
    try:
        ...
        # Fall back to a vrect without an annotation if the Plotly version
        # rejects the annotation kwargs for any reason.
        try:
            fig.add_vrect(
                x0=x0, x1=x1,
                fillcolor=c["text_muted"], opacity=0.10,
